...
十、 问题讨论
1. CPU爆满的可能场景和解决方案  
   例如：内存不足引起疯狂GC，导致CPU飙升
2. 有次调试flink程序时出现大量soft reference(软引用)，这种情况主要是什么引起的  
   解：软引用主要用于做缓存，事实上flink中也是大量使用了缓存  
3. 死锁可能会引起CPU飙升，请问什么锁会引起这种情况发生  
   解：Java中的自旋锁会发生这种情况，系统级别的锁线程只会等待  
4. 很多框架，比如Kafka对每一条消息的处理都进行了很多的判断，这样会不会影响性能  
